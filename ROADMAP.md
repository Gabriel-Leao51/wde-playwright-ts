...
- [ ] **2.** Typed test data, custom fixtures via `test.extend` (page objects, `setLanguage`), and an auth setup project using `storageState` so each role logs in once instead of in every test.
